  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await entryCreate({
        ...
        media: media.slice(0, 10).map((m) => ({ type: m.type, url: m.url })),
      })
      // duplicate: true → mismo éxito silencioso (2s y reset)
      setSuccess(true)
      setTimeout(() => {
        setTicketCode('')
        setLicensePlate('')
      ...
      // Error: se puede mostrar toast/alert si se agrega después
    } finally {
