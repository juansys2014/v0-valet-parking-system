...
import { getActive, checkoutRequest, checkoutQuickExit, ticketToVehicle } from "@/lib/api/endpoints"
...
export function CheckoutForm() {
  ...
  const loadActive = useCallback(async (search?: string) => {
    setLoading(true)
    try {
      const res = await getActive(search)
      setActiveVehicles(res.tickets.map(ticketToVehicle))
    } catch {
      setActiveVehicles([])
    } finally {
      setLoading(false)
  ...
  }, [searchTerm, loadActive])

  useEffect(() => {
    if (confirmedExit) {
      const timer = setTimeout(() => setConfirmedExit(null), 3000)
      return () => clearTimeout(timer)
    }
  }, [confirmedExit])

  const filteredVehicles = activeVehicles.filter(v => v.status === 'parked')
  const showQuickExitOption = searchTerm.trim().length >= 3 && !loading && filteredVehicles.length === 0

  const handleQRScan = async (code: string) => {
    setShowQRScanner(false)
    try {
      const res = await getActive(code.trim())
      const vehicles = res.tickets.map(ticketToVehicle)
      const found = vehicles.find(v => v.ticketCode === code.trim())
      if (found) {
  ...
  const handleConfirmRequest = async () => {
    if (!pending || pending.type !== 'registered') return
    try {
      await checkoutRequest(pending.vehicle.id)
      setConfirmedExit({
      ...
      playSound('alert')
      await loadActive(searchTerm.trim() || undefined)
    } catch {
      // Error: opcional mostrar mensaje
    }
    setPending(null)
    setSearchTerm('')
  }

  const handleConfirmQuickExit = async () => {
    if (!pending || pending.type !== 'unregistered') return
    try {
      await checkoutQuickExit({
        ticketCode: pending.ticketCode,
        licensePlate: quickExitLicensePlate.trim() || undefined,
      })
      setConfirmedExit({
        ticketCode: pending.ticketCode,
  ...
  return (
    <>
      ...
      <div className="space-y-4">
        ...
        {showQuickExitOption && (
          <Card 
            className="border-amber-500/30 bg-amber-500/5 cursor-pointer hover:shadow-md transition-all overflow-hidden"
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="font-mono font-bold text-lg truncate">#{searchTerm.trim()}</span>
                ...
                <Button variant="outline" className="gap-1 border-amber-500/50 text-amber-700 hover:bg-amber-500/10 bg-transparent flex-shrink-0 whitespace-nowrap px-3">
