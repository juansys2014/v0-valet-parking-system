...
export function VehicleDetails({ vehicle, onStatusChange, compact = false }: VehicleDetailsProps) {
  ...
  const handleMarkReady = async () => {
    ...
    setLoading(true)
    try {
      const res = await markReady(vehicle.id, attendantName || undefined)
      onStatusChange?.(ticketToVehicle(res.ticket))
    } catch {
  ...
  const handleMarkDelivered = async () => {
    ...
    setLoading(true)
    try {
      const res = await markDelivered(vehicle.id, attendantName || undefined)
      onStatusChange?.(ticketToVehicle(res.ticket))
    } catch {
      // opcional: mostrar error
    } finally {
            ...
            <div className="border-t border-border pt-4 space-y-3">
              ...
              {vehicle.status === 'requested' && (
                ...
                >
                  ...
                  {loading ? t.common.loading : t.alerts.markReady}
                </Button>
              )}
              ...
              {vehicle.status === 'ready' && (
                ...
                >
                  ...
                  {loading ? t.common.loading : t.alerts.confirmDelivery}
                </Button>
              )}
            </div>
          )}
